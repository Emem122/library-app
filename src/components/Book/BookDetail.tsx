...
export default function BookDetail({
  ...
  return (
    <div className="absolute w-4/5 h-fit max-h-3/4 sm:w-2/3 sm:max-h-2/3 max-w-lg bg-white left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 p-8 sm:p-10 overflow-auto">
      <div>
        <h2 className="">{activeBook?.title}</h2>
      </div>
      <ul className="my-5 border-dashed border-t border-b py-5 px-3 flex flex-col gap-3 [&>li]:flex">
        <li>
          <p className="w-24 text-blue-400 text-sm font-semibold">追加日</p>
          <p>{activeBook?.addedDate.substring(0, 10)}</p>
        </li>
        <li>
          <p className="w-24 text-blue-400 text-sm font-semibold">在庫</p>
          <p>{activeBook?.available ? '在庫あり' : '在庫なし'}</p>
        </li>
        <li>
          <p className="w-24 shrink-0 text-blue-400 text-sm font-semibold">
            カテゴリー
          </p>
